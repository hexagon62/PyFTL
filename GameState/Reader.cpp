...
public:
	Impl(StateReader& reader)
		...
		, processReader(*reader.processReader)
		, state(reader.state)
	...
	bool poll(bool unconditional = false)
	{
		...
		uintptr_t base = this->processReader.baseAddress();
		this->state.running = this->processReader.running();
		this->good = this->state.running;

		this->processReader.refresh();
		this->checkInGame(base);

		if (good && this->state.inGame)
		{
			this->readMisc(base);

			if (unconditional || (this->state.focused && !this->state.minimized))
			{
				this->readGui(base);

				if (unconditional || !this->state.pause.any)
				{
...
private:
	...
	{
		this->good = this->good && this->processReader.read(object, base+offset);
	}

	template<typename T>
	void read(T& object, uintptr_t base, const char* offset)
	{
		this->good = this->good && this->processReader.read(object, base + this->addr(offset));
	}

	void check(bool b)
	{
		this->good = this->good && b;
	}
	...
	void checkInGame(uintptr_t base)
	{
		...
		{
			this->processReader.refresh(true);
...
bool StateReader::hooked() const
{
	return this->processReader->running();
}

StateReader::operator bool() const
{
	return this->hooked();
}
...
const ProcessReader& StateReader::getProcessReader() const
{
	return *this->processReader;
}

void StateReader::errorCheck() const
{
	if (this->processReader->is64Bit())
		throw std::runtime_error("FTL process is 64-bit; need 32-bit version");

	if (!this->processReader->running())
	{
		int lastErr = GetLastError();
		throw std::runtime_error("Couldn't open the FTL process. Make sure the game is running.");
	}

	int lastErr = GetLastError();
	if (lastErr != 0)
		throw std::runtime_error("Windows-related error. Code: " + std::to_string(lastErr));
}

bool StateReader::poll(bool unconditional)
{
	bool succ = this->impl->poll(unconditional);
	return succ;
}
