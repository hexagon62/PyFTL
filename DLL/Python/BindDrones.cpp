...
void bindDrones(py::module_& module)
{
	...
	py::class_<Drone>(module, "Drone", "A drone")
		...
		.def_readonly("slot", &Drone::slot, "The slot the drone is in, if applicable")
		.def_readonly("blueprint", &Drone::blueprint, "The drone's blueprint")
		.def_readonly("power", &Drone::power, "Power state of the drone")
		.def_readonly("player", &Drone::player, "If the drone belongs to a player")
		.def_readonly("dead", &Drone::dead, "If the drone is dead")
		.def_readonly("dying", &Drone::dying, "If the drone is playing its dying animation")
		.def_readonly("deployed", &Drone::deployed, "If the drone is deployed")
		.def_readonly("cargo", &Drone::cargo, "If the drone is in the ship's cargo")
		.def_readonly("hack_level", &Drone::hackLevel, "The hacking status")
		.def_readonly("hack_time", &Drone::hackTime, "The time hacked/ioned")
		.def_readonly("power_up_timer", &Drone::powerUpTimer, "The progress towards the drone powering up")
		.def_readonly("power_down_timer", &Drone::powerDownTimer, "The progress towards the drone powering down")
		.def_readonly("destroy_timer", &Drone::destroyTimer, "The progress towards being able to redeploy a destroyed drone")
		.def_readonly("space", &Drone::space, "Info for if the drone is in space")
		.def_readonly("crew", &Drone::crew, "Info for if the drone is a crew drone")
		.def("powered", &Drone::powered, "Check if the drone is powered")
		.def("crew_drone", &Drone::crewDrone, "Check if the drone is a crew drone")
		.def("space_drone", &Drone::spaceDrone, "Check if the drone is a space drone")
		.def("needs_enemy", &Drone::needsEnemy, "Check if the drone needs an enemy to function")
		;
